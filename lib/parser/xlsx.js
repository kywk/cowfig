 ...
 * @return {object}         -
 */
let innerQuery = (data, range, query) => {
  let result = []
...
class Plugin {
  ...
  query (queryStr, type, args) {
    ...
    /* Full file query */
    if (!(/~|:/).test(queryStr)) {
      let _cell = getCell(data.idMap, queryStr)
    ...
    if (((rows == 1) || (cols == 1)) && !(/~~|::/).test(queryStr)) {
      result.value = []
      for (let r = cellS.cell.r; r <= cellE.cell.r; r++) {
        for (let c = cellS.cell.c; c <= cellE.cell.c; c++) {
          let _addr = XLSX.utils.encode_cell({c: c, r: r})
