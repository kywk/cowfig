...
Parser.prototype.parseFile = function (filename) {
  ...
  template.__mtime = (baseMTime > envMTime) ? baseMTime : envMTime;

  /* Prepare & keep builtin command info */
  // COPY
  if (template.__copy) {
    copy = [];
    ...
    for (let i = 0; i < template.__copy.length; i++) {
      ...
      for (let j = 0; j < files.length; j++) {
        ...
        if (filename) {
          copy.push({src: filename, dest: template.__copy[i].dest});
          break;
        }
      }
    }
  ...
  if (template.__pretty)
    pretty = template.__pretty;

  result = this.parse(template, {isRoot: true, filename: filename});

  /* Put back builtin command info */
  if (copy)
    result.__copy = copy;
  if (pretty)
    result.__pretty = pretty;
  ...
  if (!root.isRoot) {
    ...
    if (template.__macro) {
      for (let i = 0; i < template.__macro.length; i++) {
        ...
        if (_mtime > template.__mtime)
          template.__mtime = _mtime;
